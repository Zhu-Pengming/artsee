  ...
  return (
    <>
      ...
      <motion.button
        ...
        className={cn(
          "fixed bottom-40 right-8 lg:bottom-52 lg:right-12 h-14 rounded-2xl shadow-[0_10px_40px_rgba(30,58,138,0.4)] z-[100] flex items-center justify-center transition-all px-4 gap-3 group",
          isOpen 
            ? "bg-cobalt text-white w-14" 
            : "bg-gradient-to-r from-cobalt via-cobalt/90 to-ink text-white w-auto"
        )}
      >
        {/* Glowing Background Effect when visible */}
        {!isOpen && (
          <span className="absolute -inset-0.5 bg-gradient-to-r from-cobalt to-indigo-600 rounded-2xl blur opacity-30 group-hover:opacity-60 transition duration-1000 group-hover:duration-200 animate-pulse" />
        )}
          ...
          <AnimatePresence mode="wait">
            {isOpen ? (
              ...
              <motion.div
                ...
                exit={{ x: -10, opacity: 0 }}
              >
                <Sparkles size={22} className="text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.8)]" />
                <span className="text-sm font-bold tracking-tight whitespace-nowrap">AI 助手</span>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
