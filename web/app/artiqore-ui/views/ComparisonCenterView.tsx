...
export const ComparisonCenterView = ({ onBack, onInstitutionClick }: ComparisonCenterViewProps) => {
  ...
  useEffect(() => {
    ...
    fetchSchoolsForUi({ limit: 100 }).then((items) => {
      if (alive && Array.isArray(items) && items.length > 0) {
        ...
        setSelectedIds((prev) => prev.length ? prev : items.slice(0, 2).map((item) => item.id));
      }
    });
    return () => {
      alive = false;
  ...
  useEffect(() => {
    if (selectedIds.length === 0 && institutions.length >= 2) {
      setSelectedIds(institutions.slice(0, 2).map((item) => item.id));
    }
  }, [institutions, selectedIds.length]);

  const allInstitutions = useMemo(() => institutions, [institutions]);
  ...
  const comparisonPresets = [
    { name: '前二推荐', ids: allInstitutions.slice(0, 2).map((item) => item.id) },
    { name: '三校横评', ids: allInstitutions.slice(0, 3).map((item) => item.id) },
    { name: '四维组合', ids: allInstitutions.slice(0, 4).map((item) => item.id) },
  ];
              ...
              <div className="px-6 py-8 md:px-16 md:py-12 border-b border-silver/20 flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-8 bg-porcelain/30">
                ...
                <div className="flex items-center gap-4 md:gap-6">
                  <div className="relative group flex-1 md:flex-none">
                    <Search className="absolute left-4 md:left-6 top-1/2 -translate-y-1/2 text-ink/20 group-focus-within:text-cobalt transition-colors" size={16} />
                    <input 
                      type="text" 
                      placeholder="搜索院校..." 
                      value={searchQuery}
              ...
              <div className="flex-1 overflow-y-auto p-6 md:p-16 custom-scrollbar">
                {filteredInstitutions.length > 0 ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                    <Filter className="text-ink/10" size={48} />
                    <div>
