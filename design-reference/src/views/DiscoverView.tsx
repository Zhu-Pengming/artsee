...
export const DiscoverView = ({ onDetailClick }: { onDetailClick?: (item: any) => void }) => {
  ...
  return (
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="text-3xl font-serif font-bold text-ink italic">资源与机会</h2>
          <p className="text-ink/40 text-[10px] tracking-widest uppercase mt-1">Networking & Resources</p>
        </div>
        
        <div className="flex bg-silver/50 p-1.5 rounded-2xl w-full md:w-auto self-start">
          {[
            { id: 'opportunity', label: '合作机会', icon: <Briefcase size={14} /> },
            { id: 'exhibition', label: '展览空间', icon: <LayoutGrid size={14} /> },
            { id: 'artist', label: '艺术家库', icon: <Palette size={14} /> },
          ].map(t => (
            <button 
              key={t.id}
              ...
              className={cn(
                ...
                activeTab === t.id ? "bg-white text-cobalt shadow-lg" : "text-ink/40 hover:text-ink/60"
              )}
            >
              {t.icon}
              {t.label}
          ...
          <div className="space-y-10">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div 
                ...
                className="bg-ink text-white p-10 rounded-[2.5rem] flex flex-col justify-between aspect-[4/3] md:aspect-auto shadow-2xl relative overflow-hidden group cursor-pointer active:scale-95 transition-all"
              >
                 <div className="absolute top-0 right-0 w-32 h-32 bg-cobalt blur-[80px] opacity-30 group-hover:opacity-60 transition-opacity"></div>
                 <Star size={32} className="text-yellow-400 relative z-10" strokeWidth={1.5} />
                 <div className="relative z-10">
                   <h3 className="text-2xl font-serif font-bold leading-tight italic">顶奢酒店艺术招募专场</h3>
                   <p className="text-[10px] text-white/40 mt-3 uppercase tracking-[0.2em]">Premium Artistic Recruitment Series</p>
                 </div>
              </div>
              <div 
                onClick={() => onDetailClick?.({ title: '城市更新装置大赛', brand: 'CITY DESIGN COUNCIL', type: '艺术竞赛', budget: '¥1.2M', deadline: '2026.09.15' })}
                className="bg-cobalt text-white p-10 rounded-[2.5rem] flex flex-col justify-between aspect-[4/3] md:aspect-auto shadow-2xl shadow-cobalt/20 group cursor-pointer active:scale-95 transition-all"
              >
                 ...
                 <div>
                   <h3 className="text-2xl font-serif font-bold leading-tight italic">城市更新装置大赛</h3>
                   <p className="text-[10px] text-white/40 mt-3 uppercase tracking-[0.2em]">Urban Renewal Public Art Competition</p>
                 </div>
              </div>
               <div 
            ...
            <div className="space-y-8">
              <div className="flex justify-between items-center border-b border-silver/50 pb-4">
                <h3 className="text-xl font-serif font-bold text-ink">推荐机会 (Hot)</h3>
                <button className="text-xs font-bold text-cobalt hover:opacity-70 transition-opacity underline underline-offset-4">查看全部类别</button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {OPPORTUNITIES.map((opp, idx) => (
                  <motion.div 
                    ...
                    className="bg-white border border-silver/40 rounded-[2rem] p-8 shadow-sm group hover:shadow-2xl hover:shadow-cobalt/5 transition-all flex flex-col justify-between cursor-pointer active:scale-[0.98]"
                  >
                    <div>
                      <div className="flex justify-between items-start mb-6">
                        <span className="text-[9px] font-bold text-cobalt bg-cobalt/5 px-2.5 py-1 rounded-full uppercase tracking-widest border border-cobalt/10">{opp.type}</span>
                        <span className="text-[9px] text-ink/20 font-bold uppercase tracking-widest">{opp.deadline}</span>
                      </div>
                      <h4 className="text-lg font-bold text-ink group-hover:text-cobalt transition-colors mb-2 leading-tight">{opp.title}</h4>
                      <p className="text-[10px] text-ink/30 font-bold uppercase tracking-widest mb-8">{opp.brand}</p>
                    </div>
                    
                    <div className="space-y-6">
                      <div className="flex flex-wrap gap-2">
                        {opp.tags.map(t => (
                          <span key={t} className="text-[8px] font-bold text-ink/40 border border-silver/60 bg-silver/10 px-2 py-0.5 rounded-md uppercase tracking-tighter">{t}</span>
                        ))}
                      </div>
                      <div className="flex justify-between items-center pt-4 border-t border-silver/30">
                        <span className="text-xs font-serif font-bold text-ink italic">{opp.budget}</span>
                        <button className="p-2 rounded-full bg-silver/30 text-ink/40 group-hover:bg-cobalt group-hover:text-white transition-all">
        ...
        {activeTab === 'exhibition' && (
          <div className="space-y-12">
            <div 
              ...
              className="relative rounded-[3rem] overflow-hidden h-[400px] group cursor-pointer shadow-2xl active:scale-[0.99] transition-all"
            >
              <img src="https://picsum.photos/seed/exhview/1200/800" className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-1000 group-hover:scale-105" referrerPolicy="no-referrer" alt="" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
              <div className="lg:col-span-8 space-y-8">
                <div className="flex justify-between items-center border-b border-silver/50 pb-4">
                  <h3 className="text-xl font-serif font-bold text-ink italic">线下展览日历</h3>
                  ...
                  {[
                    ...
                    >
                      ...
                      <div className="flex-1 flex flex-col justify-between py-1">
                        <div>
                          <h4 className="text-base font-bold text-ink leading-tight group-hover:text-cobalt transition-colors">{exh.title}</h4>
                          <p className="text-xs text-ink/30 mt-1 uppercase tracking-tight font-medium">{exh.subtitle}</p>
                        </div>
              ...
              <div className="lg:col-span-4 bg-ink p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden flex flex-col justify-between aspect-[3/4] lg:aspect-auto">
                 ...
                 <button className="relative z-10 w-full py-4 rounded-2xl bg-white text-ink text-[10px] font-bold uppercase tracking-[0.3em] hover:bg-cobalt hover:text-white transition-all shadow-xl">订阅城市展览榜单</button>
                 <div className="absolute top-0 right-0 w-full h-full opacity-10 pointer-events-none">
                    <img src="https://picsum.photos/seed/ink/600/800" className="w-full h-full object-cover mix-blend-overlay grayscale" referrerPolicy="no-referrer" alt="" />
                 </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-8">
            {[
              ...
              <motion.div 
                ...
                className="flex flex-col group cursor-pointer active:scale-95 transition-all"
              >
                 <div className="aspect-[3/4] rounded-[2rem] overflow-hidden mb-4 relative shadow-md border border-silver/40">
                    <img src={`https://picsum.photos/seed/artp${i}/600/800`} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700 group-hover:scale-105" referrerPolicy="no-referrer" alt="" />
                 ...
                 <div className="px-2">
                   <h4 className="text-sm font-bold text-ink group-hover:text-cobalt transition-colors truncate italic">艺术家 · {artist.name}</h4>
                   <p className="text-[10px] text-ink/30 font-bold uppercase tracking-widest mt-1 truncate">{artist.major} | {artist.school}</p>
                 </div>
              </motion.div>
            ))}
          </div>
